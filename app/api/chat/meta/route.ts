	...
	try {
		...
		const matchedModels = models
			.map((model: ModelSummary): { model: ModelSummary; score: number } => {
				...
				const matchedTerms = aiTerms.filter((term) => lowerQuery.includes(term))
	...
	try {
		...
		const rankedFilters = filters
			.map((filter) => {
				...
				const desc = ((filter.description as string | null) || "").toLowerCase()
...
export async function GET(req: NextRequest) {
	try {
		...
		const topK = Number.parseInt(searchParams.get("topK") || "6", 10)
