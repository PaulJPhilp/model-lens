...
export function FilterList({ onFilterApplied }: FilterListProps) {
  ...
  const [evaluationResults, setEvaluationResults] = useState<EvaluateFilterResponse | null>(null);
  ...
  const handleSave = (savedFilter: SavedFilter) => {
    if (editingFilter) {
      ...
      setFilters(filters.map((f) => (f.id === savedFilter.id ? savedFilter : f)));
    } else {
    ...
    <div className="space-y-6 min-h-screen">
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-4 mb-4 text-sm">
                ...
                <div>
                  ...
                  <span className="font-medium">{formatDate(filter.lastUsedAt)}</span>
                </div>
              </div>
              ...
              <div className="mb-4">
                <details className="text-sm">
                  <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
                    View {filter.rules.length} rule{filter.rules.length !== 1 ? 's' : ''}
                  </summary>
                  <div className="mt-2 space-y-2 pl-4">
                    {filter.rules.map((rule, idx) => (
                      ...
                      >
                        ...
                        {rule.operator} <span className="text-primary">{String(rule.value)}</span>
                        {' '}
                        <span
                          className={
                            rule.type === 'hard'
      ...
      <Dialog open={isResultsOpen} onOpenChange={setIsResultsOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {evaluationResults?.filterName} - Results
            </DialogTitle>
            <DialogDescription>
          ...
          {evaluationResults && (
            <div className="space-y-3">
              {evaluationResults.results
                .filter((r) => r.match)
                .map((result, idx) => (
                  <div key={`${result.modelId}-${idx}`} className="p-4 border rounded-lg">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      <div>
                        <span className="text-muted-foreground">Failed Hard:</span>{' '}
                        <span className="font-medium">{result.failedHardClauses}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Passed Soft:</span>{' '}
