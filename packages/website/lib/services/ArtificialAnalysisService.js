...
export class ArtificialAnalysisService extends Context.Tag("ArtificialAnalysisService")() {
}
/**
 * Transform ArtificialAnalysis model to our internal Model format
 */
export function transformArtificialAnalysisModel(aaModel) {
    ...
    return {
        ...
        supportsAttachments: modelName.includes("vision") || modelName.includes("image"),
        new: false,
    };
}
export const ArtificialAnalysisServiceLive = {
    fetchModels: Effect.tryPromise({
        try: () => fetch("https://artificialanalysis.ai/api/datasets/96.csv").then(async (res) => {
            ...
            throw new Error(`Failed to fetch from ArtificialAnalysis: ${error instanceof Error ? error.message : "Network error"}`);
        },
    }).pipe(Effect.flatMap((models) => {
        const transformedModels = models.map(transformArtificialAnalysisModel);
        console.log(`🧠 [ArtificialAnalysisService] Fetched ${transformedModels.length} models from ArtificialAnalysis`);
