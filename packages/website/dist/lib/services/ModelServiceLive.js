...
export const ModelServiceLive = Layer.succeed(ModelService, {
    fetchModels: Effect.gen(function* () {
        ...
        const cachedModels = yield* cacheService.get(CACHE_KEYS.MODELS);
        if (cachedModels !== null) {
            console.log(`🎯 [ModelService] Using cached models (${cachedModels.length} models)`);
            return cachedModels;
        }
        ...
        const fetchResults = (yield* Effect.all([
            ...
            withRetryAndLogging(Effect.tryPromise({
                try: () => fetch("https://models.dev/api.json"),
                catch: (error) => new Error(`Failed to fetch from models.dev: ${error instanceof Error ? error.message : "Network error"}`),
            }).pipe(Effect.flatMap((response) => {
                if (!response.ok) {
                    return Effect.fail(new Error(`models.dev returned ${response.status}`));
                }
                return Effect.tryPromise({
                    try: () => response.json(),
                    catch: (error) => new Error(`Failed to parse models.dev response: ${error instanceof Error ? error.message : "Parse error"}`),
                });
            }), Effect.flatMap((data) => Effect.try({
                try: () => transformModelsDevResponse(data),
                catch: (error) => Effect.fail(new Error(`Failed to transform models.dev data: ${error instanceof Error
                    ? error.message
                    : "Transform error"}`)),
            ...
            withRetryAndLogging(Effect.tryPromise({
                ...
                catch: (error) => new Error(`Failed to fetch from OpenRouter: ${error instanceof Error ? error.message : "Network error"}`),
            }).pipe(Effect.flatMap((response) => {
                if (!response.ok) {
                    return Effect.fail(new Error(`OpenRouter returned ${response.status}`));
                }
                return Effect.tryPromise({
                    try: () => response.json(),
                    catch: (error) => new Error(`Failed to parse OpenRouter response: ${error instanceof Error ? error.message : "Parse error"}`),
                });
            }), Effect.flatMap((data) => Effect.try({
                try: () => transformOpenRouterModel(data),
                catch: (error) => Effect.fail(new Error(`Failed to transform OpenRouter data: ${error instanceof Error
                    ? error.message
                    : "Transform error"}`)),
            ...
            withRetryAndLogging(Effect.tryPromise({
                try: () => fetch("https://huggingface.co/api/models?limit=100&sort=downloads&direction=-1").then(async (res) => {
                    if (!res.ok) {
                        throw new Error(`HuggingFace returned ${res.status}`);
                    }
                    return res.json();
                }),
                catch: (error) => new Error(`Failed to fetch from HuggingFace: ${error instanceof Error ? error.message : "Network error"}`),
            }).pipe(Effect.flatMap((data) => Effect.try({
                try: () => transformHuggingFaceModel(data),
                catch: (error) => Effect.fail(new Error(`Failed to transform HuggingFace data: ${error instanceof Error
                    ? error.message
                    : "Transform error"}`)),
            ...
            withRetryAndLogging(Effect.tryPromise({
                try: () => fetch("https://artificialanalysis.ai/api/datasets/96.csv").then(async (res) => {
                    if (!res.ok) {
                        throw new Error(`ArtificialAnalysis returned ${res.status}`);
                    }
                    return res.text();
                }),
                catch: (error) => new Error(`Failed to fetch from ArtificialAnalysis: ${error instanceof Error ? error.message : "Network error"}`),
            }).pipe(Effect.flatMap((csvText) => Effect.try({
                try: () => {
                ...
                catch: (error) => Effect.fail(new Error(`Failed to transform ArtificialAnalysis data: ${error instanceof Error
                    ? error.message
                    : "Transform error"}`)),
            }))), "ArtificialAnalysis API fetch")),
        ], { concurrency: 4 }));
        // Process results and collect any errors
        const modelsDevModels = fetchResults[0]._tag === "Right" ? fetchResults[0].right : [];
        const openRouterModels = fetchResults[1]._tag === "Right" ? fetchResults[1].right : [];
        const huggingFaceModels = fetchResults[2]._tag === "Right" ? fetchResults[2].right : [];
        const artificialAnalysisModels = fetchResults[3]._tag === "Right" ? fetchResults[3].right : [];
        // Log any failures
        if (fetchResults[0]._tag === "Left") {
            console.error("❌ [ModelService] models.dev fetch failed:", fetchResults[0].left.message);
        }
        if (fetchResults[1]._tag === "Left") {
            console.error("❌ [ModelService] OpenRouter fetch failed:", fetchResults[1].left.message);
        }
        if (fetchResults[2]._tag === "Left") {
            console.error("❌ [ModelService] HuggingFace fetch failed:", fetchResults[2].left.message);
        }
        if (fetchResults[3]._tag === "Left") {
            console.error("❌ [ModelService] ArtificialAnalysis fetch failed:", fetchResults[3].left.message);
        }
        const allModels = [
            ...modelsDevModels,
            ...openRouterModels,
        ...
        console.log(`✅ [ModelService] Fetched ${allModels.length} models from external APIs (${modelsDevModels.length} from models.dev, ${openRouterModels.length} from OpenRouter, ${huggingFaceModels.length} from HuggingFace, ${artificialAnalysisModels.length} from ArtificialAnalysis)`);
        // Cache the results
        ...
        if (modelDataServiceOption._tag === "Some" && allModels.length > 0) {
            ...
            yield* Effect.fork(Effect.gen(function* () {
                ...
                if (modelsDevModels.length > 0) {
                    yield* modelDataService.storeModelBatch(modelsDevModels, syncRecord.id, "models.dev");
                }
                if (openRouterModels.length > 0) {
                    yield* modelDataService.storeModelBatch(openRouterModels, syncRecord.id, "openrouter");
                }
                if (huggingFaceModels.length > 0) {
                    yield* modelDataService.storeModelBatch(huggingFaceModels, syncRecord.id, "huggingface");
                }
                if (artificialAnalysisModels.length > 0) {
                    yield* modelDataService.storeModelBatch(artificialAnalysisModels, syncRecord.id, "artificialanalysis");
                }
                yield* modelDataService.completeSync(syncRecord.id, allModels.length, allModels.length);
            }).pipe(Effect.catchAll((error) => Effect.sync(() => console.log("⚠️ [ModelService] Background database storage failed:", error)))));
        }
        return allModels;
    }),
    fetchModelsFromAPI: withRetryAndLogging(Effect.tryPromise({
        try: () => fetch("https://models.dev/api.json").then((res) => res.json()),
        catch: (error) => new Error(`Failed to fetch from models.dev: ${error instanceof Error ? error.message : "Network error"}`),
    }).pipe(Effect.flatMap((data) => Effect.try({
        try: () => transformModelsDevResponse(data),
        catch: (error) => Effect.fail(new Error(`Failed to transform models.dev data: ${error instanceof Error ? error.message : "Transform error"}`)),
