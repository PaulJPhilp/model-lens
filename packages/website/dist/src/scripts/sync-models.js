...
const syncModelsEffect = Effect.gen(function* () {
    ...
    try {
        ...
        const [modelsDevModels, openRouterModels, huggingFaceModels, artificialAnalysisModels,] = yield* Effect.all([
            // Fetch from models.dev
            Effect.tryPromise({
                try: () => fetch("https://models.dev/api.json"),
                catch: (error) => {
                    throw new Error(`Failed to fetch from models.dev: ${error instanceof Error ? error.message : "Network error"}`);
                },
            }).pipe(Effect.flatMap((dataUnknown) => {
                const allModels = transformModelsDevResponse(dataUnknown);
                console.log(`✅ [Sync] Fetched ${allModels.length} models from models.dev`);
            ...
            Effect.gen(function* () {
                ...
                const models = yield* fetchModels;
                console.log(`✅ [Sync] Fetched ${models.length} models from HuggingFace`);
                return models;
            }),
            ...
            Effect.gen(function* () {
                ...
                const models = yield* fetchModels;
                console.log(`✅ [Sync] Fetched ${models.length} models from ArtificialAnalysis`);
                return models;
            }),
        ], { concurrency: 4 });
        // Combine all models
        ...
        console.log(`🔄 [Sync] Total transformed models: ${allModels.length} (${modelsDevModels.length} from models.dev, ${openRouterModels.length} from OpenRouter, ${huggingFaceModels.length} from HuggingFace, ${artificialAnalysisModels.length} from ArtificialAnalysis)`);
        // Store models in database with source tracking
        if (modelsDevModels.length > 0) {
            yield* modelDataService.storeModelBatch(modelsDevModels, syncId, "models.dev");
        }
        if (openRouterModels.length > 0) {
            yield* modelDataService.storeModelBatch(openRouterModels, syncId, "openrouter");
        }
        if (huggingFaceModels.length > 0) {
            yield* modelDataService.storeModelBatch(huggingFaceModels, syncId, "huggingface");
        }
        if (artificialAnalysisModels.length > 0) {
            yield* modelDataService.storeModelBatch(artificialAnalysisModels, syncId, "artificialanalysis");
        }
        // Complete sync operation
        yield* modelDataService.completeSync(syncId, allModels.length, allModels.length);
        console.log(`✅ [Sync] Successfully completed sync ${syncId}: stored ${allModels.length} models (${modelsDevModels.length} from models.dev, ${openRouterModels.length} from OpenRouter, ${huggingFaceModels.length} from HuggingFace, ${artificialAnalysisModels.length} from ArtificialAnalysis)`);
        return {
            syncId,
...
const program = Effect.gen(function* () {
    ...
    try {
        ...
        console.log(`🎉 [Sync] Model synchronization completed successfully in ${duration}ms`);
        return result;
    }
    catch (error) {
        const duration = Date.now() - startTime;
        console.error(`💥 [Sync] Model synchronization failed after ${duration}ms:`, error);
 ...
 * Run the sync script
 */
const main = program.pipe(Effect.provide(Layer.mergeAll(ModelDataServiceLive, ModelServiceLive, OpenRouterServiceLive, HuggingFaceServiceLive, ArtificialAnalysisServiceLive)), Effect.runPromise);
