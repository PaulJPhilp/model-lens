	...
	try {
		...
		try {
			...
			const modelResult = await client.query(
				"SELECT COUNT(*) as count FROM model_snapshots",
			)
			const syncResult = await client.query(
				"SELECT COUNT(*) as count FROM model_syncs",
			)
				...
				const status =
					latestSync.status === "completed"
						...
						: latestSync.status === "failed"
							...
							: "⏳"
				console.log(
					`Latest Sync: ${status} ${latestSync.status} (${new Date(latestSync.started_at).toLocaleString()})`,
				)

				if (latestSync.total_stored) {
					console.log(
						`Models Stored: ${parseInt(latestSync.total_stored).toLocaleString()}`,
					)
				}
			}
	...
	try {
      ...
      `,
			...
			for (const sync of result.rows) {
				...
				const error = sync.error_message
					? sync.error_message.substring(0, 30) + "..."
					: ""
...
function showHelp() {
	...
	console.log(
		'  bun run db query --query "SELECT COUNT(*) FROM model_snapshots"',
	)
}
