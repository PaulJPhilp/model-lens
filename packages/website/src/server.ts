		...
		HttpServerResponse.json(
			{
				error: {
					code: "NOT_FOUND",
					message:
						"Endpoint not found. Use GET /health to check server status.",
				},
				timestamp: new Date().toISOString(),
			},
			{ status: 404 },
...
console.log(
	"  GET  /v1/models                        - List all aggregated models",
)
...
const main = async () => {
	...
	const program = pipe(HttpServer.serve(app), Effect.provide(allLayers))
