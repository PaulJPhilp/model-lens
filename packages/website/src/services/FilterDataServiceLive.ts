...
} from "./FilterDataService"

/**
 * Helper function to transform database row to FilterResponse
 */
function toFilterResponse(row: typeof savedFilters.$inferSelect): FilterResponse {
	return {
		id: row.id,
		ownerId: row.ownerId,
 ...
 */
export const FilterDataServiceLive = Layer.succeed(
	FilterDataService,
	FilterDataService.of({
		listFilters: (options: FilterListOptions) =>
			Effect.tryPromise({
				try: async () => {
					...
					const query = db.select().from(savedFilters)

					let filteredQuery = query
					if (visibility && visibility !== "all") {
						filteredQuery = query.where(
							eq(savedFilters.visibility, visibility)
						) as any
					}

					// Execute query
					const filters = await filteredQuery
		...
		getFilterById: (id: string) =>
			Effect.tryPromise({
				try: async () => {
					...
					new Error(
						`Failed to get filter ${id}: ${error instanceof Error ? error.message : String(error)}`
					),
			}),
		...
		createFilter: (data: CreateFilterRequest) =>
			Effect.tryPromise({
				try: async () => {
					...
					new Error(
						`Failed to create filter: ${error instanceof Error ? error.message : String(error)}`
					),
			}),
		...
		updateFilter: (id: string, data: UpdateFilterRequest) =>
			Effect.tryPromise({
				try: async () => {
					...
					new Error(
						`Failed to update filter ${id}: ${error instanceof Error ? error.message : String(error)}`
					),
			}),
		...
		deleteFilter: (id: string) =>
			Effect.tryPromise({
				try: async () => {
					...
					return (result as { rowCount?: number }).rowCount ? true : false
				},
				catch: (error) =>
					new Error(
						`Failed to delete filter ${id}: ${error instanceof Error ? error.message : String(error)}`
					),
			}),
			...
			Effect.tryPromise({
				try: async () => {
					...
					await db
						...
						.set({
							lastUsedAt: new Date(),
							usageCount: Number(filter.usageCount) + 1,
						})
						.where(eq(savedFilters.id, id))
				},
				catch: (error) =>
					new Error(
						`Failed to increment usage for filter ${id}: ${error instanceof Error ? error.message : String(error)}`
					),
			}),
	})
)
